...
\usepackage{amsmath}
\usepackage{float}
% Control de guionación: permitir cortes razonables y mejorar justificado
% `htt` permite guionación en fuentes de tipo teletipo (código) cuando procede
\usepackage[htt]{hyphenat}
\usepackage{microtype}
\usepackage{url}
% Permitir rompimiento en barras, guiones y guiones bajos dentro de \url
...
% Ajustes para evitar overfull hboxes conservando guionación razonable
\hyphenpenalty=1000
...
\subsection{Meta de aprendizaje de la asignatura}
...
\begin{enumerate}
    ...
    \item Implementar integración con News API para obtener y mostrar noticias actualizadas sobre IoT en la interfaz web, demostrando la capacidad del ESP32 para consumir APIs REST externas y parsear respuestas JSON complejas.
    
    \item Desarrollar un sistema de caché de noticias en memoria del ESP32 que optimice el uso de la API externa y reduzca la latencia de la interfaz web.
...
\subsection{Arquitectura IoT del Sistema}
...
\subsection{News API y ArduinoJson}
...
Sensor de distancia por ultrasonidos (40 kHz) con rango de 2-400 cm y precisión de $\pm$ 3 mm. Calcula distancia mediante $d = \frac{v \cdot t}{2}$ donde $v = 343$ m/s. Requiere alimentación de 5V y pulso trigger de 10 $\mu$s. Se utiliza para determinar la posición del montacargas midiendo distancia a punto de referencia fijo.

\subsubsection{Sensor de temperatura y humedad DHT11}

Sensor digital de un solo cable que mide temperatura (0-50°C, $\pm$ 2°C) y humedad relativa (20-90\% RH, $\pm$ 5\%). Frecuencia máxima de muestreo: 1 Hz. Alimentación: 3.3-5V (compatible con ESP32). Se emplea para monitoreo de condiciones ambientales del sistema.

\subsubsection{Fotoresistor LDR}

Componente cuya resistencia varía inversamente con la luz (oscuridad: >1 M$\Omega$; luz intensa: <1 k$\Omega$). Se utiliza módulo LDR con comparador integrado que proporciona salida digital: HIGH en oscuridad, LOW con luz. Umbral ajustable mediante potenciómetro. Se emplea para detectar condiciones de iluminación y activar el LED indicador cuando es necesario.

\subsubsection{Módulo de relés HW-316}

Módulo de 4 relés que interfaz entre ESP32 y contactores del motor. Características: voltaje de control 5V, capacidad 10A @ 250VAC, optoacoplamiento para aislamiento galvánico. Control en cascada: ESP32 (GPIO 26/25/27/14 a 3.3V) → Relés HW-316 → Contactores auxiliares 24V (KA1-4) → Contactores potencia (KM) del motor trifásico.
...
\begin{itemize}
    \item \textbf{Módulo de Sensores (ESP32):} Adquisición de datos ambientales y de posición, publica telemetría vía MQTT
    \item \textbf{Broker MQTT (Mosquitto):} Intermediario de comunicaciones ejecutándose en computador portátil
    \item \textbf{Módulo de Control (ESP32):} Suscriptor MQTT, ejecuta lógica de control, interfaces web y LCD
    \item \textbf{Motor Dahlander:} Actuador principal controlado mediante módulo de 4 relés
\end{itemize}

Esta arquitectura ofrece modularidad (separación entre sensado y control), escalabilidad (fácil agregar sensores/actuadores), mantenibilidad (módulos independientes) y flexibilidad (ubicación física distribuida mediante WiFi).
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{circuito_potencia.png}
...
\begin{itemize}
    \item \textbf{KM1:} Utilizado junto con KM4 o KM5 para operación en modo lento (subida/bajada)
    \item \textbf{KM2:} Activado junto con KM3 para operación en modo rápido (debe estar desactivado KM1)
    \item \textbf{KM3:} Contactor de velocidad rápida - Se activa únicamente cuando KM2 está activado
    \item \textbf{KM4:} Contactor de subida - Trabaja en conjunto con KM1 (no puede coexistir con KM5)
    \item \textbf{KM5:} Contactor de bajada - Trabaja en conjunto con KM1 (no puede coexistir con KM4)
\end{itemize}

\textbf{Secuencias de operación:}
\begin{itemize}
    \item \textbf{Subida lenta:} Relé 1 activa KM4 + KM1
    \item \textbf{Bajada lenta:} Relé 2 activa KM5 + KM1
    \item \textbf{Modo rápido:} Relé 3 desactiva KM1 y activa KM2 + KM3
    \item \textbf{Encendido/Apagado:} Relé 4 controla alimentación general
\end{itemize}
...
\begin{itemize}
    \item \textbf{Relé 1:} Activa KM4 y KM1 para subir en modo lento
    \item \textbf{Relé 2:} Activa KM5 y KM1 para bajar en modo lento
    \item \textbf{Relé 3:} Desactiva KM1 y activa KM2 y KM3 para activar el modo rápido
    \item \textbf{Relé 4:} Activa/desactiva la alimentación para encender/apagar el sistema
\end{itemize}

\textbf{Restricciones de seguridad:}
\begin{itemize}
    \item KM4 y KM5 no pueden estar activados simultáneamente (evita conflicto de direcciones)
    \item KM1 y KM2 no pueden estar activados simultáneamente (evita cortocircuito de fases)
    \item KM3 se activa únicamente cuando KM2 está activado (secuencia de velocidad rápida)
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Integración con News API:}
    \begin{itemize}
        \item Obtiene noticias de IoT cada 5 minutos mediante HTTPS
        \item Parsea respuestas JSON (hasta 8KB) usando ArduinoJson
        \item Mantiene caché local de 5 noticias más recientes
        \item Endpoint /noticias para servir datos a la interfaz web
        \item Endpoint /actualizarnoticias para forzar actualización manual
        \item Timeout HTTP de 10 segundos para evitar bloqueos
        \item Manejo de errores HTTP (401, 429) con mensajes informativos
    \end{itemize}
\end{enumerate}
...
\subsection{APIs REST y Servicios Web}

\subsubsection{HTTP y REST}
...
\begin{itemize}
    \item \textbf{Cliente-Servidor:} Separación de responsabilidades entre interfaz de usuario y almacenamiento de datos
    \item \textbf{Sin estado:} Cada petición del cliente al servidor debe contener toda la información necesaria
    \item \textbf{Cacheable:} Las respuestas deben indicar si pueden ser cacheadas o no
    \item \textbf{Interfaz uniforme:} Recursos identificados mediante URIs, manipulados mediante representaciones
\end{itemize}
...
\begin{itemize}
    \item \textbf{Búsqueda por palabras clave:} Permite buscar artículos por términos específicos
    \item \textbf{Filtros avanzados:} Por idioma, país, categoría, fecha de publicación
    \item \textbf{Formato JSON:} Respuestas estructuradas fáciles de parsear
    \item \textbf{Plan gratuito:} 100 peticiones por día, suficiente para aplicaciones de desarrollo
\end{itemize}
...
\begin{itemize}
    \item \textbf{Bajo uso de memoria:} Optimizado para sistemas embebidos
    \item \textbf{Zero-copy:} Minimiza operaciones de copia en memoria
    \item \textbf{Streaming:} Soporte para documentos JSON grandes
    \item \textbf{Validación:} Detección de errores de formato
\end{itemize}
...
Los contactores de potencia conmuten las líneas trifásicas del motor (L1, L2, L3). En este proyecto sus funciones se definen de la siguiente forma: KM1 actúa como contactor común en los modos de velocidad lenta (se activa junto con KM4 o KM5); KM2 se emplea en la secuencia de velocidad rápida y debe permanecer desactivado cuando KM1 está activo; KM3 selecciona la configuración de velocidad rápida (menos polos); KM4 y KM5 seleccionan la configuración del sentido de giro para subida y bajada respectivamente (más polos). Las secuencias de activación incluyen enclavamientos y tiempos de espera para evitar cortocircuitos entre fases y proteger el motor.
...
En clase se realizaron pruebas prácticas para verificar el comportamiento del motor y la cadena de control (ESP32 → Relés → KA → KM) utilizando un código de prueba sencillo cargado en el módulo de control. El sketch de comprobación ejecutaba secuencias controladas que reproducían las operaciones esperadas del sistema:

\begin{itemize}
    \item Activación de alimentación (Relé 4), espera de estabilización y posterior activación del relé de dirección correspondiente (Relé 1 para subida, Relé 2 para bajada).
    \item Conmutación a velocidad alta mediante activación de Relé 3 solo tras verificar que la alimentación y la dirección estaban correctamente establecidas.
    \item Inserción de pausas y comprobaciones de enclavamientos en el código para evitar activaciones simultáneas conflictivas (p. ej. KA/KA y KM/KM incompatibles).
\end{itemize}
...
\textbf{Prueba del widget de noticias IoT:}

Se verificó exhaustivamente la integración con News API mediante múltiples pruebas. El ESP32 realizó con éxito peticiones HTTPS a newsapi.org con parámetros de búsqueda para artículos sobre IoT, "Internet of Things" y automatización en español. El tiempo de respuesta HTTP completo osciló entre 800-1500 ms, incluyendo resolución DNS, handshake TLS y transferencia de datos.
...
El mecanismo de caché funciona eficientemente: tras la primera obtención, las noticias se sirven desde memoria RAM del ESP32 con latencia inferior a 10ms, evitando consumir el límite diario de 100 peticiones de la API. La actualización automática cada 5 minutos (300 segundos) opera correctamente mediante \texttt{millis()}, y el botón de actualización manual responde inmediatamente forzando una nueva petición a la API.

La visualización web mediante JavaScript dinámico carga y renderiza las noticias correctamente en todos los navegadores probados (Chrome, Firefox, Safari, Edge). El diseño responsive adapta el formato en móviles, tablets y escritorio sin pérdida de funcionalidad.
...
\begin{enumerate}
    ...
    \item \textbf{Precisión de posicionamiento:} Error máximo de $\pm$ 2 cm en todas las plantas, cumpliendo especificación
...
\textbf{Problema:} Lecturas aisladas del HC-SR04 causaban activaciones erráticas.

\textbf{Solución:} Aplicar un filtro de mediana de 5 muestras para eliminar outliers sin penalizar la respuesta.

\subsection{Oscilación en puntos de destino}
\textbf{Problema:} Repetidas activaciones al alcanzar la planta por pequeñas variaciones del sensor.

\textbf{Solución:} Introducir un flag `destinoAlcanzado` que bloquea nuevos comandos hasta un nuevo objetivo.
\subsection{Latencia en comunicación MQTT}
\textbf{Problema:} Intervalo de publicación demasiado largo que provocaba sobrepasos en movimientos rápidos.

\textbf{Solución:} Reducir frecuencia de publicación (ej. 2 s → 250 ms) para mejorar la reacción del control.
...
\begin{enumerate}
    ...
    \item \textbf{Control de posición preciso:} El algoritmo de control logra posicionamiento con precisión de $\pm$ 2 cm en las cuatro plantas definidas, cumpliendo la especificación establecida.
...
\item Intervalo de muestreo: 2s para todas las mediciones
\item Filtro de mediana de 5 muestras para distancia
\item Validación de rango: 2-400 cm
\item Bibliotecas: PubSubClient, DHT sensor library, Adafruit Unified Sensor
\end{itemize}
...
\textbf{Software y servicios:}
\begin{itemize}
    \item Broker MQTT: Mosquitto (gratuito, open source)
    \item News API: Plan gratuito (100 peticiones/día)
    \item Arduino IDE: Gratuito
    \item Bibliotecas: PubSubClient, DHT, LiquidCrystal I2C, ArduinoJson (todas gratuitas)
\end{itemize}

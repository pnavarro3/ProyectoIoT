...
Se emplea un módulo de 4 relés HW-316 para control de cargas de potencia (motor Dahlander).
...
Los relés permiten al ESP32 controlar circuitos de potencia del motor trifásico sin riesgo de daño por sobrevoltaje o corrientes elevadas, proporcionando aislamiento eléctrico entre la lógica de control y la carga.
...
\subsubsection{Control de velocidad mediante relés}

En este proyecto, se emplean 4 relés para controlar el motor Dahlander:

\begin{enumerate}
    \item \textbf{Relé 1 (GPIO 26):} Control de subida a velocidad lenta - Activa la configuración de más polos para movimiento ascendente controlado
    \item \textbf{Relé 2 (GPIO 25):} Control de bajada a velocidad lenta - Activa la configuración de más polos para movimiento descendente controlado
    \item \textbf{Relé 3 (GPIO 27):} Activación de velocidad alta - Conmuta el motor a la configuración de menos polos para desplazamiento rápido
    \item \textbf{Relé 4 (GPIO 14):} Alimentación general del motor - Control maestro de potencia al sistema motriz
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Activa/desactiva relés según la lógica de control
    \end{itemize}
    
    \item Los \textbf{relés} conmutan las conexiones del motor Dahlander según comandos recibidos.
...
El control del motor Dahlander constituye uno de los elementos centrales del sistema, permitiendo el desplazamiento del montacargas con velocidad variable según la fase del movimiento.

\subsubsection{Configuración de relés}

Se emplea un módulo de 4 relés HW-316 conectado al ESP32 mediante las siguientes asignaciones:

\begin{table}[H]
\centering
\begin{tabular}{|c|c|l|}
\hline
\textbf{Relé} & \textbf{Pin GPIO} & \textbf{Función} \\
\hline
Relé 1 & GPIO 26 & Control motor - Subida lenta \\
Relé 2 & GPIO 25 & Control motor - Bajada lenta \\
Relé 3 & GPIO 27 & Control motor - Velocidad alta \\
Relé 4 & GPIO 14 & Control motor - Alimentación general \\
\hline
\end{tabular}
\caption{Asignación de relés para control del motor Dahlander}
\end{table}

Los relés actúan sobre los contactores del motor, reconfigurando las conexiones de los bobinados para lograr las dos velocidades características del motor Dahlander. La configuración específica depende del tipo de motor (Y/YY o $\Delta$/YY) y el esquema de cableado implementado.
